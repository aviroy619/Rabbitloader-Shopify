...
class RabbitLoaderDashboard {
  ...
  initializeAppBridge() {
    ...
    const shop = this.shop || urlParams.get('shop');
    const host = this.host || urlParams.get('host');
    const embedded = this.embedded || (urlParams.get('embedded') === '1');
    ...
    const isEmbedded = window.top !== window.self || embedded;
    
    if (isEmbedded && shop && window.env.SHOPIFY_API_KEY && host) {
      try {
        const AppBridge = window['app-bridge'];
        if (AppBridge && AppBridge.createApp) {
          // Use the provided host parameter directly
  ...
  setupEventListeners() {
    ...
    if (urlParams.get('script_injected') === '1') {
      this.showSuccess('Script automatically injected into your theme!');
    }
  }
  ...
  showFlashMessage(message, type = 'info') {
    if (!this.flashMessages) {
      ...
      this.flashMessages = document.createElement('div');
      this.flashMessages.id = 'flashMessages';
      document.body.appendChild(this.flashMessages);
    }
